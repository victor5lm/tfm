...
This repository contains all scripts and auxiliary data used to analyse metagenomic data from two cohorts, perform differential abundance analyses (DAA) and build all Machine Learning classifiers, as well as regarding the validation of one of these ML models and the figures shown in this MSc Thesis' memory.
...
This directory contains all files used for the training of all ML classifiers, for the generation of both heatmaps shown in the TFM Thesis and for the plotting of all figures shown in the same document. Other data relative to ML validation and taxonomic assignment are also available here. These files are:
...
- `rooted-tree.qza`: this is one of the QIIME2 artefacts needed to create phyloseq objects from the study cohort necessary for diversity analyses, DAA and ML classifier construction. It is the rooted tree obtained during read processing with QIIME2, MAFFT and FastTree.
- `taxonomy.qza`: this is one of the QIIME2 artefacts needed to create phyloseq objects from the study cohort necessary for diversity analyses, DAA and ML classifier construction. If transformed into a visualisation file (.qzv), it informs us about the number of samples taken into account, the number of identified features, etc. If transformed into a visualisation file (.qzv), it informs us about the full taxonomy of each identified feature.
...
- One directory called `additional_files_hei_classification`. Three files are stored here:
    ...
    - `groups_for_heatmap_genuses_hei.csv`: this file consists in 20 rows and 2 columns: rows are for each one of the 20 most important taxa according to the best classifier based on this classification of the study cohort, while the first column are these taxa and the second one indicates the HEI group in which they are enriched.
...
- `alpha_and_beta_diversity.R`: R script describing all procedures needed to calculate alpha and beta diversity measures and plot several indices for both cohorts and classifications.
...
- `boxplots.R`: R script indicating in detail how boxplots showing taxa being significantly different in their CLR-transformed relative abundances between HPF- or HEI-based groups were made (Supp. Figs 3, 10 and 11). It is therefore indicated how those individuals with type 2 diabetes family history were highlighted, as well as how the Wilcoxon rank sum test was used to find these differentially abundant taxa between groups.
...
- `importance_barplots.R`: R script describing how Supplementary Figures 2 and 9, consisting in barplots and showing the most important taxa according to each one of our ML classifiers, were made.
- `metaphlan_to_R.R`: R script indicating how, once the relative abundances table from the validation cohort was obtained after executing MetaPhlAn4, these abundaces were imported into R for the creation of all phyloseq objects necessary for diversity analyses, DAA and ML classifier validation.
...
- `population_tables.R`: R script describing how Tables 1-3, which showed some demographic and additional data regarding both cohorts in the MSc Thesis, were created using the gtsummary R package.
- `qiime_to_R.R`: R script indicating how, once all QIIME2-related procedures for the study cohort were finished, the resulting artefacts were used for the creation of phyloseq objects in R necessary for diversity analyses, DAA and ML classifier construction.
- `quantile_graphs.R`: R script describing how Supplementary Figures 12 to 15, showing boxplots of some differently abundant taxa vs quantiles made using the validation cohort based on their levels of several glycaemic traits (HOMA-IR, HbA1c (%) and adiponectin) and HEI, were obtained. The identification of these taxa differentially abundant among these quantiles via the Kruskal-Wallis rank sum test is also detailed in this script.
- `roc_curves.R`: R script detailing how ROC curves for all ML classifiers regarding both classifications of the study cohort were made (Figures 3 and 5, as well as Supplementary Figures 1 and 8).
- `validation.R`: R script describing how validation of the best ML classifier trained on the study cohort classified based on their HEI was performed using the relative abundances table from the validation cohort, along with the plotting of the ROC curve linked to this validation (Figure 6 of the MSc Thesis).
