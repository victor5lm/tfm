...
data_complete_ias_groups_rf <- read.table('C:\\Users\\USUARIO\\Desktop\\data_clr_complete_ias_groups.tsv',
                      header = TRUE, sep = '\t',# skip = 1,
                      row.names = 1)
descrCorr <- cor(data_complete_ias_groups_rf)
highCorr <- findCorrelation(descrCorr, 0.90)
data_rf.uncor <- data_complete_ias_groups_rf[ , -highCorr]

ntrees<-seq(1,100)
tuning_rf<-list(mtry=seq(1,100))
originals_ias_rf <- list()

for (ntree in ntrees){print(ntree)
    set.seed(2019)
    results <- run_ml(data_rf.uncor, "rf",
    outcome_colname = 'Group_HPF',
    ...
    key<-toString(ntree)
    originals_ias_rf[[key]]<-results
}

#Best model
originals_ias_rf$"88"$performance #AUC = 0.85

#Variable importances
imp.bi <- varImp(originals_ias_rf$"88",scale=FALSE)
...
table_imp_mikropml_rf <- read.csv("DATA/importances_HEI/imp_mikropml_rf_ias.csv")
