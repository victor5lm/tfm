...
ggplot(data = tabla_prueba, aes(x = `HPF group`, y = Abundance, label = SampleID, group = `HPF group`, fill = `HPF group`)) +
    ...
    stat_summary(fun = "mean", geom = "point", aes(shape = "Mean"), size = 2, color = "gold", show.legend = TRUE) + geom_text_repel(aes(label=SampleID),
                                                                                                                                    color = ifelse(tabla_prueba$SampleID %in% c("PYM107","PYM202","PYM502","PYM511","PYM310","PYM402","PYM404"), "white", NA),     # text color
                                                                                                                                    bg.color = ifelse(tabla_prueba$SampleID %in% c("PYM107","PYM202","PYM502","PYM511","PYM310","PYM402","PYM404"), "grey30", NA), # shadow color
                                                                                                                                    bg.r = ifelse(tabla_prueba$SampleID %in% c("PYM107","PYM202","PYM502","PYM511","PYM310","PYM402","PYM404"), 0.15, NA),          # shadow radius
                                                                                                                                    max.overlaps = Inf,
                                                                                                                                    point.size = 8,
                                                                                                                                    min.segment.length = 0,
                                                                                                                                    segment.color = ifelse(tabla_prueba$SampleID %in% c("PYM107","PYM202","PYM502","PYM511","PYM310","PYM402","PYM404"), "black", NA)) +
    ...
    geom_signif(comparisons = list(c("Low HPF consumption", "High HPF consumption")),
                test = "wilcox.test", map_signif_level = TRUE, textsize = 5, fontface = "bold")+
    theme(
