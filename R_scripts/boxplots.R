...
# Boxplot of some microbial genera whose abundances are significantly different between HPF consumption groups (Study cohort)
    ...
    set.seed(2019)
    ggplot(data = tabla_prueba, aes(x = `HPF_group`, y = Abundance, label = SampleID, group = `HPF_group`, fill = `HPF_group`)) +
        geom_boxplot() +
...
# Boxplot of some microbial genera whose abundances are significantly different between HEI consumption groups (Study cohort)
    ...
    tabla_prueba_hei <- tb_clr[tb_clr$Genus %in% c(" Butyricimonas", " CAG-352", " Candidatus_Soleaferrea", " Intestinimonas"), ] #tb_clr is the object obtained after using psmelt on the phyloseq object.
    tabla_prueba_subset_hei <- subset(tabla_prueba_hei,SampleID %in% c("PYM107", "PYM202", "PYM502", "PYM511", "PYM310", "PYM402", "PYM404"))
    set.seed(2019)
    ggplot(data = tabla_prueba_hei, aes(x = `HEI_group`, y = Abundance, label = SampleID, group = `HEI_group`, fill = `HEI_group`)) +
        geom_boxplot() +
        labs(x = "Group", y = "Abundance\n") +
        facet_wrap(~ Genus, scales = "free") + 
        stat_summary(fun = "mean", geom = "point", aes(shape = "Mean"), size = 2, color = "gold", show.legend = TRUE) + geom_text_repel(aes(label=SampleID),
        color = ifelse(tabla_prueba_hei$SampleID %in% c("PYM107", "PYM202","PYM502","PYM511","PYM310","PYM402","PYM404"), "white", NA),     # text color
        bg.color = ifelse(tabla_prueba_hei$SampleID %in% c("PYM107", "PYM202","PYM502", "PYM511", "PYM310", "PYM402", "PYM404"), "grey30", NA), # shadow color
        bg.r = ifelse(tabla_prueba_hei$SampleID %in% c("PYM107", "PYM202", "PYM502", "PYM511", "PYM310", "PYM402", "PYM404"), 0.15, NA),          # shadow radius
        max.overlaps = Inf,
        point.size = 8,
        min.segment.length = 0,
        segment.color = ifelse(tabla_prueba_hei$SampleID %in% c("PYM107", "PYM202", "PYM502", "PYM511", "PYM310", "PYM402", "PYM404"), "black", NA)) +
        geom_point(color = ifelse(tabla_prueba_hei$SampleID %in% c("PYM107", "PYM202", "PYM502", "PYM511", "PYM310", "PYM402", "PYM404"), "green", "grey")) +
